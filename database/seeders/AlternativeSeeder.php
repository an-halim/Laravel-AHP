...
class AlternativeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        \Log::error($criteriaValues, $insertedIds);
        // Insert criteria values into the Comparisons table
        try {
            Comparisons::insert($criteriaValues);
        } catch (\Exception $e) {
            \Log::error("Error inserting criteria values: {$e->getMessage()}", $criteriaValues);
        }
    }
    ...
    private function processCriteria(array $requestData, $alternativeId)
    {
        ...
        foreach ($requestData as $key => $value) {
            $criteria = Criteria::whereRaw('LOWER(name) = ?', [strtolower($key)])->first();

            if ($criteria) {
                \Log::warning("Criteria not found for key: {$key}");

                $subCriteriaId = $this->findMatchingSubCriteria($criteria->id, $value);

                if ($subCriteriaId) {
                    \Log::warning("SubCriteria not found for Criteria ID: {$criteria->id} and Value: {$value}");
                    $criteriaValues[] = [
    ...
    {
        $subs = SubCriteria::where('criteria_id', $criteriaId)
            ->orderBy('value', 'asc')
            ->get();
        ...
        foreach ($subs as $sub) {
            ...
            // Track the closest sub-criteria for fallback
            if ($sub->operator == '>=' && (int) $sub->value <= $value) {
                $closestSub = $sub;
            }
        }

        // Fallback to the closest match if no exact match was found
        return $closestSub ? $closestSub->id : null;
    }
}
